...
class MyPrayerViewModel: ObservableObject {
    ...
    func deletePrayer(prayerID: Int) async {
        do {
            ...
            var prayers = writtenPrayers
            prayers.removeAll { $0.id == prayerID }
            self.writtenPrayers = prayers
        } catch {
    ...
    func deletePrayerResponse(responseID: Int) async {
        do {
            ...
            var prayers = participatedPrayers
            prayers.removeAll { $0.id == responseID }
            self.participatedPrayers = prayers
        } catch {
            let error = error.localizedDescription
            alertType = .error(title: "삭제", message: error)
        }
